...
def process_worker_function(
    ...
    results = {"processed": 0, "awaiting_tags": 0, "failed": 0}
    ...
    for _ in range(max_iterations):
        ...
        photo_path = state_db.claim_next_photo(
            worker_id, include_awaiting_tags=force_process
        )
        if not photo_path:
...
def process_single_photo(
    ...
    try:
        ...
        if not current["ai_response_json"]:
            ...
            pending_tags = [
                tag for tag in analysis.new_tags_needed if tag not in approved_tags
            ]

            if pending_tags and not force_process:
                # Store pending tags for review
                for tag in pending_tags:
                    tag_storage.add_pending_tag(tag, filepath, analysis.confidence)
                ...
                logger.info(
                    f"Photo {photo_path.name} awaiting {len(pending_tags)} tag approvals"
                )
                ...
                for tag in pending_tags:
                    tag_storage.add_pending_tag(tag, filepath, analysis.confidence)
                logger.info(
                    f"Processing {photo_path.name} without {len(pending_tags)} pending tags"
                )
        ...
        if current["status"] == PhotoStatus.AI_ANALYZED.value:
            ...
            approved_tags = tag_storage.get_approved_tag_names()
            tags_to_write = [
                tag for tag in analysis.existing_tags_used if tag in approved_tags
            ]
...
class PhotoProcessor:
    ...
    def process_batch(
        ...
        Returns per-run counts plus 'started_at' (UTC string) for stats scoping.
        """
        ...
        if continue_session:
            ...
            self.session_id = state_db.create_session(worker_count=self.worker_count)
        ...
        if duplicate_count:
            logger.info(
                f"Diverted {duplicate_count} duplicates straight to {self.outbox_dir}"
            )
            ...
            try:
                ...
                while pending and not self._shutdown_requested:
                    ...
                    if progress_callback:
                        progress_callback(
                            state_db.get_statistics(since=run_started_utc)
                        )
    ...
    def _divert_duplicate(
        ...
        logger.info(
            f"Duplicate {photo_path.name} (same content as {known_as}) diverted to outbox"
        )
        return True
